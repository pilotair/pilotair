...
export default function LeftMenuLayout({ sider, content, header }: Props) {
    ...
    return (
        <Layout className="absolute inset-0">
            <Resizable className={resizing ? "!transition-none" : ""} width={resizableWidth} onResize={onResize} axis="x" handle={<div className="absolute w-1 top-0 bottom-0 right-[-2px] cursor-ew-resize hover:bg-sky-500 z-[99999]" />} onResizeStart={onResizeStart} onResizeStop={onResizeStop}>
